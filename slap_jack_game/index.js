...
let discardCards = [];
let playerCards = [];
const message_a = document.querySelector('.message_a');
const message_b = document.querySelector('.message_b');
...
function messageOutputA(mes){
    message_a.innerHTML = mes;
}
function messageOutput_b(mes){
    message_b.innerHTML = mes;
}
...
function playCard(event) {
    ...
    message_a.style.display = `inline-block`;
	if (target === `player-deck`) {
        messageOutput( [playerCards.length -1]);
        messageOutputA(`Cards in Play:` + [discardCards.length + 1]);
		discardCards.push(playerCards[0]);
		playerCards.splice(0, 1);

	} else if (target === `opponent-deck`) {
        messageOutput_b([opponentCards.length-1]);
        messageOutputA(`Cards in Play:` + [discardCards.length + 1]);
...
function opponentAI(lastPlayer) {
    ...
    reaction = window.setTimeout(function() {
        ...
        if (discardCardsLength > 0 && discardCards[discardCardsLength - 1].includes(`J`)) {
            messageOutputA("Slap! opponent wins"+  [discardCardsLength] + " " +`cards`);
            slap();
...
function slap(event) {
    ...
    if (event !== undefined) {
        ...
        messageOutputA(`slap! you win:` + [discardCardsLength] + `cards`);
    } else {
